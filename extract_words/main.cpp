...
void extractWords(std::string const& s, OutputIterator out)
{
    static auto const isSpace = [](char letter){ return letter == ' '; };
		...
		auto lastExaminedPosition = s.cbegin();
		while (lastExaminedPosition != s.cend())
		{
			auto const beginWord = std::find_if_not(lastExaminedPosition, s.cend(), isSpace);
			auto const endWord = std::find_if(beginWord, s.cend(), isSpace);
